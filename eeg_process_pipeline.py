...
class EEGProcessor:
    ...
    def compare_filter(self, duration=10):
        ...
        print("\n=== 对比滤波前后信号 ===")
        # 绘制原始信号
        self.raw.plot(duration=duration, n_channels=8, scalings='auto', 
                     title='滤波前 (Original Raw)', show=False)
        # 绘制滤波后信号
        self.raw_filtered.plot(duration=duration, n_channels=8, scalings='auto', 
                             title='滤波后 (Filtered)', show=False)
    ...
    def compare_ica(self, duration=10):
        ...
        # ICA处理前 (即滤波后)
        self.raw_filtered.plot(duration=duration, n_channels=8, scalings='auto', 
                             title='ICA处理前 (Filtered)', show=False)
        # ICA处理后
        self.raw_clean.plot(duration=duration, n_channels=8, scalings='auto', 
                          title='ICA处理后 (Cleaned)', show=False)
